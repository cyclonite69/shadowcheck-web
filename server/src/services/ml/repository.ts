...
type ThreatScoreRow = {
  ...
  ml_feature_values?: Record<string, unknown>;
...
async function loadThreatModelConfig() {
...
async function loadNetworksForLegacyScoring(limit: number) {
  const result = await query(
    `
      SELECT
        ap.bssid,
...
async function upsertLegacyThreatScore(score: ThreatScoreRow, overwriteFinal?: boolean) {
  if (overwriteFinal) {
    await query(
      `
...
async function getMLModelStatus(): Promise<any> {
  ...
  return {
    ...
    mlScoresCount: parseInt(scoreRows.rows[0]?.count || 0, 10),
  };
}

async function getMLTrainingData(): Promise<any[]> {
...
async function getMLScoreForNetwork(bssid: string): Promise<any | null> {
  const result = await query('SELECT * FROM app.network_threat_scores WHERE bssid = $1', [bssid]);
  return result.rows.length > 0 ? result.rows[0] : null;
}

async function getNetworksByThreatLevel(level: string, limit: number): Promise<any[]> {
  const result = await query(
    `
...
async function getNetworksForBehavioralScoring(
  limit: number,
