...
export function inferRadioType(
  ...
  if (ssidUpper.includes('LTE') || ssidUpper.includes('4G') || capUpper.includes('LTE') || capUpper.includes('EARFCN')) return 'L';
  if (ssidUpper.includes('WCDMA') || ssidUpper.includes('3G') || ssidUpper.includes('UMTS') || capUpper.includes('WCDMA') || capUpper.includes('UMTS') || capUpper.includes('UARFCN')) return 'D';
  if (ssidUpper.includes('GSM') || ssidUpper.includes('2G') || capUpper.includes('GSM') || capUpper.includes('ARFCN')) return 'G';
  ...
  if (ssidUpper.includes('[UNKNOWN / SPOOFED RADIO]') || ssidUpper.includes('BLE') || ssidUpper.includes('BTLE') || capUpper.includes('BLE') || capUpper.includes('BTLE') || capUpper.includes('BLUETOOTH LOW ENERGY')) return 'E';
  ...
  if (capUpper.includes('WPA') || capUpper.includes('WEP') || capUpper.includes('WPS') || capUpper.includes('RSN') || capUpper.includes('ESS') || capUpper.includes('CCMP') || capUpper.includes('TKIP')) return 'W';
  return '?';
}
...
const buildFeatureCollection = <TRow>(
  ...
  features: rows
    .filter((row) => { const [lon, lat] = getCoordinates(row); return lon !== null && lat !== null; })
    .map((row) => {
      const [lon, lat] = getCoordinates(row);
      return { type: 'Feature', geometry: { type: 'Point', coordinates: [lon, lat] }, properties: mapProperties(row) };
    }),
});

export const buildKeplerDataGeoJson = (rows: KeplerNetworkRow[], rowCount: number | null | undefined) =>
  buildFeatureCollection(rows, rowCount, (row) => [row.lon, row.lat], (row) => ({
    bssid: row.bssid, ssid: row.ssid || 'Hidden Network', bestlevel: row.signal || 0, signal: row.signal || 0, level: row.signal || 0,
    manufacturer: row.manufacturer || 'Unknown', type: inferRadioType(row.type, row.ssid, row.frequency, row.capabilities),
    threat_level: row.threat?.level || null, threat_score: row.threat?.score || null,
    observation_count: row.observations || 0, obs_count: row.observations || 0, unique_days: row.unique_days || 0,
    max_distance_meters: row.max_distance_meters || 0, first_observed_at: row.first_observed_at, last_observed_at: row.last_observed_at,
    geocoded_address: row.geocoded_address, geocoded_poi_name: row.geocoded_poi_name,
  }));

export const buildKeplerObservationsGeoJson = (rows: KeplerObsRow[], rowCount: number | null | undefined) =>
  buildFeatureCollection(rows, rowCount, (row) => [row.lon, row.lat], (row) => {
    ...
    return {
      bssid: row.bssid, ssid: row.ssid || 'Hidden Network', bestlevel: row.level || 0, signal: row.level || 0, level: row.level || 0,
      manufacturer: row.manufacturer || 'Unknown', type: inferRadioType(row.radio_type, row.ssid, row.radio_frequency, row.radio_capabilities),
      threat_level: row.threat_level || null, threat_score: row.threat_score || null,
      observation_count: row.observations || 0, obs_count: row.observations || 0, unique_days: row.unique_days || 0,
      max_distance_meters: row.max_distance_meters || 0, max_distance_km: (row.max_distance_meters || 0) / 1000,
      first_observed_at: row.first_observed_at, last_observed_at: row.last_observed_at,
      timespan_days: Math.round(timespanDays * 10) / 10, stationary_confidence: row.stationary_confidence,
      geocoded_address: row.geocoded_address, geocoded_poi_name: row.geocoded_poi_name,
    };
  });

export const buildKeplerNetworksGeoJson = (rows: KeplerNetworkRow[], rowCount: number | null | undefined) =>
  buildFeatureCollection(rows, rowCount, (row) => [row.lon, row.lat], (row) => ({
    bssid: row.bssid, ssid: row.ssid || 'Hidden Network', bestlevel: row.signal || 0, signal: row.signal || 0, level: row.signal || 0,
    manufacturer: row.manufacturer || 'Unknown', type: inferRadioType(row.type, row.ssid, row.frequency, row.capabilities),
    threat_level: row.threat?.level || null, threat_score: row.threat?.score || null,
