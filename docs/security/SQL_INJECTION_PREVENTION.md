...
const result = await query('SELECT * FROM app.networks_legacy WHERE bssid = $1', [bssid]);

// Good: Multiple parameters
const result = await query('SELECT * FROM app.networks_legacy WHERE type = $1 AND rssi > $2', [
  networkType,
  minSignal,
]);
...
const result = await query('SELECT * FROM app.networks_legacy WHERE ssid LIKE $1', [
...
const result = await query(`SELECT * FROM networks WHERE type = '${type}'`);

// Bad: String concatenation
const result = await query('SELECT * FROM networks WHERE ssid = ' + ssid);
```
...
describe('SQL Injection Prevention', () => {
  it('should escape LIKE patterns', async () => {
    const response = await request(app).get('/api/networks/search/%test_value').expect(200);
  ...
  it('should reject malicious BSSID', async () => {
    const response = await request(app).get("/api/networks/'; DROP TABLE networks;--").expect(400);
...
const ssidSchema = Joi.string().max(32).required();
```
...
const result = await query(`SELECT * FROM networks WHERE ssid LIKE $1 ESCAPE E'\\\\'`, [pattern]);
```
...
const result = await query(`SELECT * FROM networks ${whereClause}`, params);
```
...
const result = await query(`SELECT * FROM networks ORDER BY ${sortColumn} ${order}`, []);
```
