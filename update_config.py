...
sql = f"UPDATE app.settings SET value = '{json.dumps(config)}'::jsonb WHERE key = 'geocoding_daemon_config';"
command = f"sudo docker exec shadowcheck_postgres env PGPASSWORD=zJ346bJvKOoyPq9B3bVfmcBXhyycbnkQ psql -U shadowcheck_admin -d shadowcheck_db -c \"{sql}\""
