...
cmd_search() {
  ...
  local response=$(api_call "/v1/networks?search=${query}")
  ...
  echo "$response" | jq '.networks[] | {ssid, bssid, signal, type, observations}' 2>/dev/null || echo "$response"
}
...
cmd_threats() {
  ...
  echo "$response" | jq '.threats[] | {ssid, bssid, threatScore, threatLevel}' 2>/dev/null || echo "$response"
}
...
cmd_network() {
  ...
  local response=$(api_call "/v1/networks/${bssid}")
...
cmd_type() {
  ...
  local response=$(api_call "/v1/networks?type=${type}")
  ...
  echo "$response" | jq '.networks[] | {ssid, bssid, signal, observations}' 2>/dev/null || echo "$response"
}
