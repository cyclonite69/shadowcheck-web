-- Fix NULL timestamp issue in materialized view
-- Filter out records with NULL timestamps that cause sorting issues

DROP VIEW IF EXISTS public.api_network_explorer CASCADE;
DROP MATERIALIZED VIEW IF EXISTS public.api_network_explorer_mv;

-- Recreate with NULL timestamp filtering
...
-- ============================================================================
-- MAIN SELECT: Complete API Contract with IMPROVED SECURITY PARSING
-- ============================================================================
SELECT
  ap.bssid,
  COALESCE(
  ...
  -- Fix NULL timestamp issue by using COALESCE with obs.time as fallback
  COALESCE(
    CASE
  ...
  CASE
    ...
    -- WPA3 SAE (Simultaneous Authentication of Equals) - Personal
    ...
    -- WPS only (without other security)
...
WHERE COALESCE(ap.is_sentinel, FALSE) = FALSE
  -- Filter out records with NULL timestamps to fix sorting issues
  AND (
