-- Migration: Enforce UPPERCASE SSID across all tables
-- Ensures all SSIDs are stored in uppercase for consistency

BEGIN;

-- Step 1: Create trigger function to automatically uppercase SSIDs
CREATE OR REPLACE FUNCTION public.uppercase_ssid_trigger()
RETURNS TRIGGER AS $$
BEGIN
  -- Convert SSID to uppercase if not null
  IF TG_TABLE_NAME = 'access_points' AND NEW.latest_ssid IS NOT NULL THEN
    NEW.latest_ssid = UPPER(NEW.latest_ssid);
  ELSIF TG_TABLE_NAME IN ('observations', 'ssid_history') AND NEW.ssid IS NOT NULL THEN
...
-- Step 2: Create triggers for access_points table
DROP TRIGGER IF EXISTS trigger_uppercase_ssid_access_points ON public.access_points;
CREATE TRIGGER trigger_uppercase_ssid_access_points
  BEFORE INSERT OR UPDATE OF latest_ssid ON public.access_points
  FOR EACH ROW
  EXECUTE FUNCTION public.uppercase_ssid_trigger();

-- Step 3: Create triggers for observations table
...
-- Step 4: Create triggers for ssid_history table
...
-- Step 5: Update existing SSIDs to uppercase in access_points
...
-- Step 6: Update ssid_variants array to uppercase
UPDATE public.access_points
...
WHERE ssid_variants IS NOT NULL AND array_length(ssid_variants, 1) > 0;

-- Step 7: Update existing SSIDs to uppercase in observations
UPDATE public.observations
SET ssid = UPPER(ssid)
WHERE ssid IS NOT NULL
  AND ssid != UPPER(ssid);

-- Step 8: Update existing SSIDs to uppercase in ssid_history
UPDATE public.ssid_history
SET ssid = UPPER(ssid)
WHERE ssid IS NOT NULL
  AND ssid != UPPER(ssid);

-- Step 9: Create indexes for faster SSID searches (if they don't exist)
...
-- Add comments
COMMENT ON FUNCTION public.uppercase_ssid_trigger() IS 'Automatically converts all SSIDs to uppercase before insert/update';
COMMENT ON TRIGGER trigger_uppercase_ssid_access_points ON public.access_points IS 'Ensures latest_ssid is always uppercase';
...
BEGIN
  ...
  RAISE NOTICE '✓ SSID Uppercase Migration Complete:';
