...
-- WHY: Reset derived objects to ensure reproducible rebuilds without touching raw ingest tables.
DROP VIEW IF EXISTS public.api_network_explorer CASCADE;
DROP MATERIALIZED VIEW IF EXISTS public.api_network_explorer_mv;

-- WHY: Materialize heavy, forensic-grade computations outside UI paths for deterministic results.
...
-- WHY: Unique index enables REFRESH MATERIALIZED VIEW CONCURRENTLY and ensures 1 row per BSSID.
CREATE UNIQUE INDEX IF NOT EXISTS api_network_explorer_mv_bssid_idx
  ON public.api_network_explorer_mv (bssid);

-- WHY: Support UI-critical filters and ordering without touching base tables.
...
-- WHY: Keep API contract stable while forcing reads through the materialized view.
...
-- WHY: Populate materialized view data after schema creation.
