-- ==========================================================================
-- BASELINE 006: Indexes, Grants, and Defaults
-- ==========================================================================
...
-- - 20260412_orphan_search_index_and_promotion.sql


-- ======== SOURCE: 20260216_consolidated_010_performance_indexes.sql ========
...
-- ======== SOURCE: 20260404_add_geocoding_to_api_network_explorer_mv.sql ========
...
network_geo AS (
  SELECT
    n.bssid,
    COALESCE(nloc.weighted_lat, nloc.centroid_lat, bo.lat) AS rep_lat,
    COALESCE(nloc.weighted_lon, nloc.centroid_lon, bo.lon) AS rep_lon
  FROM app.networks n
  LEFT JOIN best_obs bo ON n.bssid = bo.bssid
...
network_geocode AS (
  ...
  LEFT JOIN app.geocoding_cache gc
    ON gc.precision = 5
   AND ng.rep_lat IS NOT NULL
   AND ng.rep_lon IS NOT NULL
   AND gc.lat_round = round(ng.rep_lat::numeric, 5)
   AND gc.lon_round = round(ng.rep_lon::numeric, 5)
)
SELECT
  n.bssid,
  n.ssid,
...
GROUP BY
  ...
  ngc.geocoded_address, ngc.geocoded_city, ngc.geocoded_state, ngc.geocoded_postal_code,
  ngc.geocoded_country, ngc.geocoded_poi_name, ngc.geocoded_poi_category,
  ngc.geocoded_feature_type, ngc.geocoded_provider, ngc.geocoded_confidence;
...
-- ======== SOURCE: 20260412_orphan_search_index_and_promotion.sql ========
