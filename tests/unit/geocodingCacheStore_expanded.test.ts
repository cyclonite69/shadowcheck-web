...
describe('geocodingCacheStore', () => {
  ...
  describe('shouldReplaceAddressData', () => {
    test('handles valid incoming data correctly', () => {
      ...
      expect(
        shouldReplaceAddressData(
          { address: 'A', confidence: 0.5 },
          { ok: true, address: 'B', confidence: 0.7 }
        )
      ).toBe(true);
      expect(
        shouldReplaceAddressData(
          { address: 'A', confidence: 0.5, provider: 'opencage' },
          { ok: true, address: 'B', confidence: 0.51, provider: 'locationiq' }
        )
      ).toBe(true);
    });
    ...
    test('rejects bad incoming data', () => {
      ...
      expect(
        shouldReplaceAddressData(
          { address: 'A', confidence: 0.8 },
          { ok: true, address: 'B', confidence: 0.5 }
        )
      ).toBe(false);
    });
  });
