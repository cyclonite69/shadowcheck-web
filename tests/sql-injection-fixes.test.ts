...
describe('SQL Injection Prevention', () => {
  ...
  describe('NetworkRepository.getDashboardMetrics() - Config Parameterization', () => {
    ...
    test('treats invalid-only radioTypes selection as neutral instead of throwing', async () => {
      query
        .mockResolvedValueOnce({ rows: [{ total_networks: '5', wifi_count: '1' }] })
        .mockResolvedValueOnce({ rows: [{ total_observations: '10', wifi_observations: '2' }] })
      ...
      await expect(
        repo.getDashboardMetrics({ radioTypes: ['INVALID'] }, { radioTypes: true })
      ).resolves.toMatchObject({ totalNetworks: 5, totalObservations: 10 });
